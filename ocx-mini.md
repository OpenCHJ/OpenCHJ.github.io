...
OCX mini は、OpenCHJに収録するために、文学作品・史料・資料テキストを**形態素解析・検索しやすい形でアノテーションするための、最小限のマークアップ方式**です。
...
-   TEIやXHTMLなどのXML文書にも、プレーンテキストにも、同じ方法でアノテーションを追加できる
...
- 元のテキスト構造をほとんど変えない
- 人手で修正・校訂しやすい
- 将来、OCX standard、TEI などのXML文書に変換しやすい
...
`<head>` のタイトルやメタ情報は解析対象になりません。下記の例の`<div class="bibliographical_information">`以下も囲まないようにすれば解析対象になりません。
...
**最終的に「OCX mini 準拠」とする文書では必須**です。この境界が「Web茶まめ」や「中納言」などでも文境界として使われます。
...
本文に出さないで注記を残すためのタグです。

``` xml
<ocx:comment text="外字を通用字に改めた"/>
```
...
OCX mini は TEIを拡張するものではありません。しかしTEIとして完成した文書にアノテーションを加えてコーパス用の解析処理を行う際に有効に使えます。
...
より詳細な情報の付与や解析時辞書選択などの制御を必要とする場合はOCX standardを利用してください。

------------------------------------------------------------------------

## 8. まとめ

-   本文文字は全て解析対象になり、解析後の全ての処理の起点になる（しっかり校正し、確定したら全角スペースや記号を含めていっさい変更しない）
-   文は必ず `<ocx:eos/>` で切る
-   読み・注記は **本文に混ぜない**
