...
テキストデータをアノテーションする際は、次のようにデフォルト名前空間を`https://openchj.github.io/ns/oc`とすることで名前空間接頭辞（`ocx:`）を省略できます。
...
スキーマを用いてXMLファイルの検証する場合は冒頭のocx:docタグに下記のようにスキーマロケーションを指定します。
``` xml
<ocx:doc textID="八木重吉　貧しき信徒" corpusName="青空文庫"
...
**最終的に「OCX mini 準拠」とする文書では必須**です。この境界が「中納言」などでも文境界として使われます。
